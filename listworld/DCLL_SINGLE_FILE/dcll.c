...
node_t *locate_node(list_t *p_list, int search_data); 
node_t *get_list_node(int new_data); 
...
int pop_beg(list_t *p_list, int *pp_data)
{
    if(is_list_empty(p_list) == TRUE)
    {
        return (LIST_EMPTY); 
...
int pop_end(list_t *p_list, int *pp_data)
{
    if(is_list_empty(p_list) == TRUE)
    {
        return (LIST_EMPTY); 
...
int list_to_array(list_t *p_list, int **pp_array, size_t *p_size){
    ...
    p_array = (int*)xmalloc(list_len * sizeof(int)); 
