...
void dijiktras(struct vertex v[],int n_v,struct edge e[],int n_e,int src){
    v[src].d=0;
    int visited[n_v];//keeping the track of the all vertex visited or not
    for (int i=0;i<n_v;i++){
        visited[i]=0;
    }
    while (true){
        if (check(visited,n_v)){
            break;
        }
        int min=696969;
        for (int i=0;i<n_v;i++){
            if (visited[i]==0 && min>v[i].d){
                min=i;
            }
        }
        visited[min]=1;
        cout<<"minimum selected vertex is"<<min<<"\n";
        for (int i=0;i<n_e;i++){
            if (e[i].src==min){
                if (v[e[i].dest].d>(v[e[i].src].d+e[i].weight)){
                    v[e[i].dest].d=v[e[i].src].d+e[i].weight;
                    v[e[i].dest].pred=e[i].src;
                    cout<<"true here"<<"\n";
                }
            }
        }
...
void prim(struct vertex v[],int n_v,struct edge e[],int n_e,int src){
    v[src].d=0;
    int visited[n_v];//keeping the track of the all vertex visited or not
    for (int i=0;i<n_v;i++){
        visited[i]=0;
...
void kruskal(struct vertex v[],int n_v,struct edge e[],int n_e,int src){
}
int main(){
    int n_v;
